...
const PeriodChartComponent: React.FC<PeriodChartProps> = ({
  ...
  initialData,
  onLoadMoreData,
  periodType = 'week', // 기본값은 일봉
  apiBaseUrl = '/api',
}) => {
  // 초기 데이터 (일별 데이터)
  const { data: stockDailyData, isLoading, isError } = useStockDailyData(companyId ?? 1, 2, 50);
...
export const WeekChart = React.memo<PeriodChartProps>(PeriodChartComponent, arePropsEqual);
